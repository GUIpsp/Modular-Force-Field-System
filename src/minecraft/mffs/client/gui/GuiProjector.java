...
public class GuiProjector extends GuiContainer {
		...
		if ((i != 1) && (this.editMode)) {
			...
			if (i == 14) {
				NetworkHandlerClient
						.fireTileEntityEvent(this.projector, 12, "");
			}
			if ((i != 54) && (i != 42) && (i != 58) && (i != 14)) {
				NetworkHandlerClient.fireTileEntityEvent(this.projector, 11,
						String.valueOf(c));
			}
		} else {
			super.keyTyped(c, i);
	...
	protected void mouseClicked(int i, int j, int k) {
		...
		if (this.editMode) {
			...
			NetworkHandlerClient
					.fireTileEntityEvent(this.projector, 10, "null");
			this.editMode = true;
		}
	}
	...
	protected void drawGuiContainerBackgroundLayer(float f, int i, int j) {
		...
		if (this.projector.hasValidTypeMod()) {
			...
			if ((ProjectorTypes.typeFromItem(this.projector.getType()).ProTyp != 4)
					&& (ProjectorTypes.typeFromItem(this.projector.getType()).ProTyp != 2)) {
				drawTexturedModalRect(w + 155, k + 63, 177, 143, 16, 16);
			}

			if ((ProjectorTypes.typeFromItem(this.projector.getType()).ProTyp == 1)
					|| (ProjectorTypes.typeFromItem(this.projector.getType()).ProTyp == 2)
					|| (ProjectorTypes.typeFromItem(this.projector.getType()).ProTyp == 6)
					|| (ProjectorTypes.typeFromItem(this.projector.getType()).ProTyp == 7)
					|| (ProjectorTypes.typeFromItem(this.projector.getType()).ProTyp == 8)) {
			...
			if (this.projector.hasOption(
					ModularForceFieldSystem.itemOptionCamouflage, true)) {
	...
	@Override
	public void initGui() {
		this.controlList.add(new GraphicButton(1, this.width / 2 + 4,
				this.height / 2 - 37, this.projector, 1));
		this.controlList.add(new GraphicButton(0, this.width / 2 + 67,
				this.height / 2 - 88, this.projector, 0));
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		this.fontRenderer.drawString(this.projector.getDeviceName(), 12, 9,
				4210752);
		...
		if (this.projector.hasPowerSource()) {
			this.fontRenderer.drawString(
					String.valueOf(this.projector.getLinkPower()), 30, 80,
					4210752);
		} else {

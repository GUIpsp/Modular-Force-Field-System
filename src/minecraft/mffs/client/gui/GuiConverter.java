...
public class GuiConverter extends GuiContainer {
		...
		if ((i != 1) && (this.NameeditMode)) {
			...
			if (i == 14) {
				NetworkHandlerClient
						.fireTileEntityEvent(this.Converter, 12, "");
			}
			if ((i != 54) && (i != 42) && (i != 58) && (i != 14)) {
				NetworkHandlerClient.fireTileEntityEvent(this.Converter, 11,
						String.valueOf(c));
			}
		} else {
			super.keyTyped(c, i);
	...
	protected void mouseClicked(int i, int j, int k) {
		...
		if (this.NameeditMode) {
			...
			NetworkHandlerClient
					.fireTileEntityEvent(this.Converter, 10, "null");
	...
	protected void actionPerformed(GuiButton guibutton) {
		NetworkHandlerClient.fireTileEntityEvent(this.Converter, guibutton.id,
				"null");
	}

	@Override
	public void initGui() {
		this.controlList.add(new GraphicButton(0, this.width / 2 + 107,
				this.height / 2 - 104, this.Converter, 0));

		if (MFFSConfiguration.MODULE_IC2) {
			this.controlList.add(new GraphicButton(100, this.width / 2 - 25,
					this.height / 2 - 80, this.Converter, 1));
		}
		if (MFFSConfiguration.MODULE_UE) {
			this.controlList.add(new GraphicButton(101, this.width / 2 + 50,
					this.height / 2 - 80, this.Converter, 2));
		}
		super.initGui();
	}
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		this.fontRenderer.drawString(this.Converter.getDeviceName(), 100, 8,
				4210752);
		...
		if (MFFSConfiguration.MODULE_IC2) {
			...
			this.fontRenderer.drawString(
					"" + this.Converter.getIC_Outputpacketsize(), 103, 48,
					16777215);
			this.fontRenderer.drawString("x", 132, 48, 16777215);
			this.fontRenderer.drawString(
					"" + this.Converter.getIC_Outputpacketamount(), 150, 48,
					16777215);
		}
		if (MFFSConfiguration.MODULE_UE) {
			...
			this.fontRenderer.drawString(
					"" + this.Converter.getUE_Outputvoltage(), 180, 48,
					16777215);
			this.fontRenderer.drawString("" + this.Converter.getUE_Outputamp(),
					222, 48, 16777215);
		}

		if (this.Converter.getPowerSourceID() != 0) {
			this.fontRenderer.drawString(
					"FE: " + this.Converter.getLinkPower(), 17, 54, 4210752);
		} else {

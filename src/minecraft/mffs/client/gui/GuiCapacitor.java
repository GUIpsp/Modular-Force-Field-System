...
public class GuiCapacitor extends GuiMFFS {
	...
	public void initGui() {
		...
		this.controlList.add(new GraphicButton(0, this.width / 2 + 65,
				this.height / 2 - 100, this.tileEntity, 0));
		this.controlList.add(new GraphicButton(1, this.width / 2 + 5,
				this.height / 2 - 35, this.tileEntity, 1));
	...
	protected void drawGuiContainerBackgroundLayer(float f, int x, int y) {
		 ...
		 * Frequency Card Slot
		 */
		this.drawSlot(8, 73, new ItemStack(
	...
	protected void drawGuiContainerForegroundLayer(int x, int y) {
		this.fontRenderer.drawString(this.tileEntity.getInvName(), this.ySize
				/ 2 - this.tileEntity.getInvName().length() * 5, 6, 4210752);

		this.drawTextWithTooltip("linkedDevice",
				"%1: " + this.tileEntity.getLinketProjector(), 8, 30, x, y);
		this.drawTextWithTooltip("range",
				"%1: " + this.tileEntity.getTransmitRange(), 8, 45, x, y);
		this.drawTextWithTooltip("frequency", "%1:", 8, 60, x, y);
		this.textFieldFrequency.drawTextBox();
		this.drawTextWithTooltip("forcePower", "%1:", 8, 95, x, y);
		this.fontRenderer.drawString(
				ElectricInfo.getDisplay(
						this.tileEntity.getStorageAvailablePower(),
						ElectricUnit.JOULES)
						+ "/"
						+ ElectricInfo.getDisplay(
								this.tileEntity.getMaximumPower(),
								ElectricUnit.JOULES), 8, 105, 4210752);

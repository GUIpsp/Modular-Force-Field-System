...
public class GuiAreaDefenseStation extends GuiContainer {
	...
	public GuiAreaDefenseStation(EntityPlayer player,
			TileEntityDefenseStation tileentity) {
	...
	protected void keyTyped(char c, int i) {
		if ((i != 1) && (this.editMode)) {
			...
			if (i == 14) {
				NetworkHandlerClient.fireTileEntityEvent(this.DefenceStation,
						12, "");
			}
	...
	protected void mouseClicked(int i, int j, int k) {
		...
		if (this.editMode) {
			...
			NetworkHandlerClient.fireTileEntityEvent(this.DefenceStation, 10,
					"null");
			this.editMode = true;
		}
	}
	...
	public void initGui() {
		this.controlList.add(new GraphicButton(0, this.width / 2 + 107,
				this.height / 2 - 104, this.DefenceStation, 0));
		this.controlList.add(new GraphicButton(100, this.width / 2 + 47,
				this.height / 2 - 38, this.DefenceStation, 1));
		this.controlList.add(new GraphicButton(101, this.width / 2 - 36,
				this.height / 2 - 58, this.DefenceStation, 2));
		this.controlList.add(new GraphicButton(102, this.width / 2 + 20,
				this.height / 2 - 58, this.DefenceStation, 3));
		super.initGui();
	}
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		this.fontRenderer.drawString(this.DefenceStation.getDeviceName(), 120,
				9, 4210752);
		...
		if (this.DefenceStation.getPowerSourceID() != 0) {
			this.fontRenderer.drawString(
					"FE: " + this.DefenceStation.getCapacity() + " %", 35, 31,
					4210752);
		} else {
		...
		this.fontRenderer.drawString(
				"perimeter: " + this.DefenceStation.getInfoDistance(), 12, 73,
				4210752);

		this.fontRenderer.drawString("action", 35, 91, 15612731);
		this.fontRenderer.drawString(
				"perimeter: " + this.DefenceStation.getActionDistance(), 12,
				111, 4210752);

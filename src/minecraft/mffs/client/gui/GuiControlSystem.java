...
public class GuiControlSystem extends GuiContainer {
	...
	protected void keyTyped(char c, int i) {
		if ((i != 1) && (this.editMode)) {
			...
			if (i == 14) {
				NetworkHandlerClient.fireTileEntityEvent(this.ControlSystem,
						12, "");
			}
	...
	protected void mouseClicked(int i, int j, int k) {
		...
		if (this.editMode) {
			...
			NetworkHandlerClient.fireTileEntityEvent(this.ControlSystem, 10,
					"null");
			this.editMode = true;
		}
	}
	...
	protected void actionPerformed(GuiButton guibutton) {
		if (guibutton.id == 103) {
			NetworkHandlerClient.fireTileEntityEvent(this.ControlSystem,
					guibutton.id, this.player.username);
		} else {
			NetworkHandlerClient.fireTileEntityEvent(this.ControlSystem,
					guibutton.id, "");
		}
	...
	@Override
	public void initGui() {
		this.controlList.add(new GraphicButton(100, this.width / 2 - 115,
				this.height / 2 - 45, this.ControlSystem, 1));
		this.controlList.add(new GraphicButton(101, this.width / 2 - 115,
				this.height / 2 - 25, this.ControlSystem, 2));
		this.controlList.add(new GraphicButton(102, this.width / 2 - 115,
				this.height / 2 - 5, this.ControlSystem, 3));
		this.controlList.add(new GuiButton(103, this.width / 2 + -65,
				this.height / 2 - 8, 100, 20, "Open Remote Gui"));
		super.initGui();
	}
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		...
		this.fontRenderer.drawString(this.ControlSystem.getDeviceName(), 123,
				9, 4210752);
		...
		this.fontRenderer.drawString(
				"Name: " + this.ControlSystem.getRemoteDeviceName(), 15, 30,
				4210752);
		this.fontRenderer.drawString(
				"Type:  " + this.ControlSystem.getRemoteDeviceTyp(), 15, 45,
				4210752);
		if (this.ControlSystem.getStackInSlot(1) != null) {
			...
			renderItem.renderItemIntoGUI(this.mc.fontRenderer,
					this.mc.renderEngine, new ItemStack(
							ModularForceFieldSystem.itemCardSecurityLink), 40,
					59);
			renderItem
					.renderItemIntoGUI(this.mc.fontRenderer,
							this.mc.renderEngine, new ItemStack(
			...
			if (this.ControlSystem.getRemotehasPowersource()) {
				...
				this.fontRenderer.drawString("Power left: "
						+ this.ControlSystem.getRemotePowerleft() + "%", 40,
						80, 4210752);
			} else {

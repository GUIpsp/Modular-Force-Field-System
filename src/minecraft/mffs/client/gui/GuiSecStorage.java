...
public class GuiSecStorage extends GuiContainer {
	...
	protected void keyTyped(char c, int i) {
		if ((i != 1) && (this.editMode)) {
			...
			if ((i != 54) && (i != 42) && (i != 58) && (i != 14)) {
				NetworkHandlerClient.fireTileEntityEvent(this.SecStorage, 11,
						String.valueOf(c));
			}
		} else {
			super.keyTyped(c, i);
	...
	public void initGui() {
		this.controlList.add(new GraphicButton(0, this.width / 2 + 65,
				this.height / 2 - 113, this.SecStorage, 0));
		super.initGui();
	}
	...
	protected void mouseClicked(int i, int j, int k) {
		...
		if (this.editMode) {
			...
			NetworkHandlerClient.fireTileEntityEvent(this.SecStorage, 10,
					"null");
			this.editMode = true;
		}
	}
	...
	protected void drawGuiContainerForegroundLayer(int par1, int par2) {
		this.fontRenderer.drawString(this.SecStorage.getDeviceName(), 12, 9,
				4210752);

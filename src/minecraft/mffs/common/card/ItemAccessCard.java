...
public class ItemAccessCard extends ItemCardPersonalID {
	...
	public void addInformation(ItemStack itemStack, EntityPlayer player,
		...
		String SecurityArea = String.format("Security Area: %s ",
				new Object[] { getforAreaname(itemStack) });
		info.add(SecurityArea);

		String validity = String.format("period of validity: %s min",
				new Object[] { Integer.valueOf(getvalidity(itemStack)) });
		info.add(validity);
		NBTTagCompound rightsTag;
		if ((Keyboard.isKeyDown(42)) || (Keyboard.isKeyDown(54))) {
			rightsTag = NBTTagCompoundHelper.getTAGfromItemstack(itemStack)
					.getCompoundTag("rights");
			info.add("Rights:");

...
public class ItemAccessCard extends ItemCardPersonalID
{
    ...
    {
        String SecurityArea = String.format("Security Area: %s ", new Object[]
        {
            getforAreaname(itemStack)
        });
        info.add(SecurityArea);

        String validity = String.format("period of validity: %s min", new Object[]
        {
            Integer.valueOf(getvalidity(itemStack))
        });
        info.add(validity);

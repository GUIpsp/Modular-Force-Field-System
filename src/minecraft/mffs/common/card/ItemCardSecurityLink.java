...
public class ItemCardSecurityLink extends ItemCard {
	...
	public void onUpdate(ItemStack itemStack, World world, Entity entity,
		...
		if (this.tick > 600) {
			...
			if (Sec_ID != 0) {
				TileEntitySecurityStation sec = (TileEntitySecurityStation) FrequencyGrid
						.getWorldMap(world).getSecStation()
						.get(Integer.valueOf(Sec_ID));
				if (sec != null) {
	...
	public static TileEntitySecurityStation getLinkedSecurityStation(
			...
			if ((inventiory.getStackInSlot(slot).getItem() instanceof ItemCardSecurityLink)) {
				...
				if (png != null) {
					...
					if ((world.getBlockTileEntity(png.X, png.Y, png.Z) instanceof TileEntitySecurityStation)) {
						TileEntitySecurityStation sec = (TileEntitySecurityStation) world
						...
						if (sec != null) {
							if ((sec.getDeviceID() == card.getValuefromKey(
								...
								if (!sec.getDeviceName().equals(
										getforAreaname(inventiory
												.getStackInSlot(slot)))) {
									setforArea(inventiory.getStackInSlot(slot),
											sec.getDeviceName());
								}
								return sec;
							}
						...
						if (Sec_ID != 0) {
							...
							if (sec != null) {
								card.setInformation(
										inventiory.getStackInSlot(slot),
										sec.getMachinePoint(), "Secstation_ID",
										Sec_ID);
								return sec;
							}
						}
					...
					if (world.getChunkFromBlockCoords(png.X, png.Z).isChunkLoaded) {
						inventiory.setInventorySlotContents(slot,
								new ItemStack(

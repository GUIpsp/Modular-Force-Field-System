...
public class ItemModuleTube extends ItemModule3DBase {
	...
	public void calculateField(IModularProjector projector, Set ffLocs,
		...
		int distance = projector
				.countItemsInSlot(IModularProjector.Slots.Distance) + 2;
		int Strength = projector
				.countItemsInSlot(IModularProjector.Slots.Strength);
		...
		for (int z1 = 0 - tpz + z_offset_s; z1 <= tpz - z_offset_e; z1++) {
			for (int x1 = 0 - tpx + x_offset_s; x1 <= tpx - x_offset_e; x1++) {
				for (int y1 = 0 - tpy + y_offset_s; y1 <= tpy - y_offset_e; y1++) {
					...
					if ((tpx == Strength)
							&& ((projector.getSide() == 4) || (projector
									.getSide() == 5))) {
						tpx_temp++;
					}
					if ((tpy == Strength)
							&& ((projector.getSide() == 0) || (projector
									.getSide() == 1))) {
						tpy_temp++;
					}
					if ((tpz == Strength)
							&& ((projector.getSide() == 2) || (projector
									.getSide() == 3))) {
						tpz_temp++;
					}

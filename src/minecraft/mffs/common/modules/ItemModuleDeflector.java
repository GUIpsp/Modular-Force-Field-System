	...
	public void calculateField(IModularProjector projector, Set ffLocs) {
		...
		for (int x1 = 0 - projector
			...
			for (int z1 = 0 - projector
					.countItemsInSlot(IModularProjector.Slots.FocusUp); z1 < projector
					.countItemsInSlot(IModularProjector.Slots.FocusDown) + 1; z1++) {
				if (projector.getSide() == 0) {
					tpy = 0 - projector
							.countItemsInSlot(IModularProjector.Slots.Distance) - 1;
					tpx = x1;
					tpz = z1;
				}

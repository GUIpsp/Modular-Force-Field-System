...
public class ItemModuleAdvancedCube extends ItemModule3DBase {
	...
	public void calculateField(IModularProjector projector, Set ffLocs,
		...
		int Strength = projector
				.countItemsInSlot(IModularProjector.Slots.Strength) + 2;
				...
				for (int z1 = 0 - zPout; z1 < zMout + 1; z1++) {
					...
					if ((y1 == 0) || (y1 == Strength) || (x1 == 0 - xMout)
						...
						if (((TileEntityProjector) projector)
								.hasOption(
										ModularForceFieldSystem.itemOptionFieldManipulator,
										true)) {
							switch (((TileEntityProjector) projector).getSide()) {
							case 0:
								if (((TileEntityProjector) projector).yCoord
										+ tpy <= ((TileEntityProjector) projector).yCoord) {
									break;
								}
								break;
							case 1:
								...
								break;
							case 2:
								if (((TileEntityProjector) projector).zCoord
										+ tpz <= ((TileEntityProjector) projector).zCoord) {
									break;
								}
								break;
							case 3:
							...
							case 4:
							case 5:
								if (((TileEntityProjector) projector).xCoord
										+ tpx > ((TileEntityProjector) projector).xCoord
										&& ((TileEntityProjector) projector).xCoord
												+ tpx < ((TileEntityProjector) projector).xCoord) {
									continue;
								}
							}

	...
	public ContainerSecStorage(EntityPlayer player,
		...
		for (var3 = 0; var3 < 6; var3++) {
			for (int var4 = 0; var4 < 9; var4++) {
				addSlotToContainer(new SlotHelper(this.SecStorage, var4 + var3
						* 9 + 1, 12 + var4 * 18, 43 + var3 * 18));
			}
		...
		for (var3 = 0; var3 < 3; var3++) {
			for (int var4 = 0; var4 < 9; var4++) {
				addSlotToContainer(new Slot(player.inventory, var4 + var3 * 9
						+ 9, 12 + var4 * 18, 155 + var3 * 18));
			}

		}

		for (var3 = 0; var3 < 9; var3++) {
			addSlotToContainer(new Slot(player.inventory, var3, 12 + var3 * 18,
					213));
		}
	}
		...
		if ((slot != null) && (slot.getHasStack())) {
			...
			if (i < this.SecStorage.getSizeInventory()) {
				if (!mergeItemStack(itemstack1,
						this.SecStorage.getSizeInventory(),
						this.inventorySlots.size(), true)) {
					return null;
				}
			} else if (!mergeItemStack(itemstack1, 0,
					this.SecStorage.getSizeInventory(), false)) {
				return null;
			}

...
public class TileEntityControlSystem extends TileEntityMFFS implements
		ISidedInventory {
	...
	public void invalidate() {
		FrequencyGrid.getWorldMap(this.worldObj).getControlSystem()
		...
		if ((key == 103) && (this.remote != null) && (getRemoteGUIinRange())) {
			...
			if (player != null) {
				player.openGui(ModularForceFieldSystem.instance, 0,
						this.worldObj, this.remote.xCoord, this.remote.yCoord,
						this.remote.zCoord);
			}
	...
	private void refreshRemoteData(int slot) {
		...
		if (this.remote != null) {
			...
			if (!this.remote.getDeviceName().equalsIgnoreCase(
					getRemoteDeviceName())) {
			...
			if (!MachineTypes.fromTE(this.remote).getName()
					.equalsIgnoreCase(getRemoteDeviceTyp())) {
				setRemoteDeviceTyp(MachineTypes.fromTE(this.remote).getName());
			}

			if ((PointXYZ.distance(getMachinePoint(),
					this.remote.getMachinePoint()) > 61.0D)
					&& (getRemoteGUIinRange())) {
	...
	private TileEntityMFFS getTargetMaschine(int slot) {
		if ((getStackInSlot(slot) != null)
			...
			if (DeviceID != 0) {
				TileEntityMFFS device = FrequencyGrid
						.getWorldMap(this.worldObj)

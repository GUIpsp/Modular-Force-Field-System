...
public class TileEntitySecurityStation extends TileEntityMFFS {
	...
	public void invalidate() {
		FrequencyGrid.getWorldMap(this.worldObj).getSecStation()
	...
	public void checkslots() {
		if (getStackInSlot(0) != null) {
			if (getStackInSlot(0).getItem() == ModularForceFieldSystem.itemCardID) {
				...
				if (ItemCardPersonalID.hasRight(getStackInSlot(0),
						SecurityRight.CSR) != true) {
					ItemCardPersonalID.setRight(getStackInSlot(0),
							SecurityRight.CSR, true);
				}
			} else {
				setMainUser("");
		...
		if ((getStackInSlot(39) != null)
				&& (getStackInSlot(38) != null)
				&& ((getStackInSlot(38).getItem() instanceof ItemCardEmpty))
				&& ((getStackInSlot(39).getItem() instanceof ItemCardPersonalID))) {
			setInventorySlotContents(38,
	...
	public boolean RemoteInventory(String username, SecurityRight right) {
		for (int a = 35; a >= 1; a--) {
			if ((getStackInSlot(a) != null)
					&& (getStackInSlot(a).getItem() == ModularForceFieldSystem.itemCardID)) {
				String username_invtory = NBTTagCompoundHelper
						.getTAGfromItemstack(getStackInSlot(a)).getString(
								"name");

				ItemCardPersonalID Card = (ItemCardPersonalID) getStackInSlot(a)
						.getItem();

				boolean access = ItemCardPersonalID.hasRight(getStackInSlot(a),
						right);
						...
						if (ItemAccessCard.getvalidity(stack) > 0) {
							if (ItemAccessCard.getlinkID(stack) == getDeviceID()) {
								if (ItemAccessCard.hasRight(stack, right)) {
									if (!ItemAccessCard.getforAreaname(stack)
											.equals(getDeviceName())) {
		...
		case 102:
			if (getStackInSlot(1) != null) {
				if ((getStackInSlot(1).getItem() instanceof ItemCardEmpty)) {
					setInventorySlotContents(1, new ItemStack(
							ModularForceFieldSystem.itemCardAccess, 1));

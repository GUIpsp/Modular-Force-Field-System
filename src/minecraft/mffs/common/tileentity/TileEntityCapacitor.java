...
public class TileEntityCapacitor extends TileEntityForcePowerMachine implements
	...
	public TileEntitySecurityStation getLinkedSecurityStation() {
		return ItemCardSecurityLink.getLinkedSecurityStation(this, 4,
				this.worldObj);
	}
	...
	public int getStorageMaxPower() {
		if ((getStackInSlot(0) != null)
				&& (getStackInSlot(0).getItem() == ModularForceFieldSystem.itemUpgradeCapacity)) {
	...
	private void checkSlots() {
		...
		if (getStackInSlot(2) != null) {
			if ((getStackInSlot(2).getItem() instanceof IForceEnergyItems)) {
									...
									ForceEnergyItem
											.setAvailablePower(
													...
													ForceEnergyItem
															.getAvailablePower(getStackInSlot(2))
															+ maxtransfer);
									setForcePower(getStorageAvailablePower()
											- maxtransfer);
								} else {
									ForceEnergyItem
											.setAvailablePower(
				...
				case 4:
					if (ForceEnergyItem.getAvailablePower(getStackInSlot(2)) > 0) {
						int maxtransfer = ForceEnergyItem
								.getPowerTransferrate();
						int freeeamount = getStorageMaxPower()
								- getStorageAvailablePower();
			...
			if (getStackInSlot(2).getItem() == ModularForceFieldSystem.itemCardPowerLink) {
				if ((getPowerLinkMode() != 0) && (getPowerLinkMode() != 1)
						&& (getPowerLinkMode() != 2))
					setPowerLinkMode(0);
			}
		}
	...
	public void updateEntity() {
		if (!this.worldObj.isRemote) {
			...
			if (getTicker() == 10) {
				if (getLinketProjector().shortValue() != (short) FrequencyGrid
						.getWorldMap(this.worldObj).connectedtoCapacitor(this,
								getTransmitRange())) {
					setLinketprojektor(Short.valueOf((short) FrequencyGrid
							.getWorldMap(this.worldObj).connectedtoCapacitor(
									this, getTransmitRange())));
				}
				if (getPercentageStorageCapacity() != getStorageAvailablePower()

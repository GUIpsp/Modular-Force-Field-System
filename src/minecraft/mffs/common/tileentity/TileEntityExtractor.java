...
public class TileEntityExtractor extends TileEntityForcePowerMachine implements
	...
	public TileEntityExtractor() {
		...
		if (MFFSConfiguration.MODULE_BUILDCRAFT) {
			this.powerProvider = PowerFramework.currentFramework
					.createPowerProvider();
			this.powerProvider.configure(10, 2,
					(int) (getMaxWorkEnergy() / 2.5D),
					(int) (getMaxWorkEnergy() / 2.5D),
					(int) (getMaxWorkEnergy() / 2.5D));
		}
	}
	...
	private boolean hasStuffToConvert() {
		...
		if (getStackInSlot(0) != null) {
			...
			if ((getStackInSlot(0).getItem() == ModularForceFieldSystem.itemForcicumCell)
					&& (((ItemForcicumCell) getStackInSlot(0).getItem())
		...
		if (!this.worldObj.isRemote) {
			...
			if (getTicker() >= getWorkTicker()) {
				...
				if ((this.workmode == 0) && (isActive())) {
					if (getWorkDone() != getWorkEnergy() * 100
							/ getMaxWorkEnergy()) {
					...
					if (getCapacity() != getForceEnergybuffer() * 100
	...
	public boolean isItemValid(ItemStack itemStack, int slot) {
		switch (slot) {
		case 0:
			if ((((itemStack.getItem() instanceof ItemForcicium)) || ((itemStack
		...
		case 3:
			...
			break;
		case 4:
			if (((itemStack.getItem() instanceof ItemForcicumCell))
	...
	public void invalidate() {
		...
		FrequencyGrid.getWorldMap(this.worldObj).getExtractor()
	...
	public void convertMJtoWorkEnergy() {
		if (getWorkEnergy() < getMaxWorkEnergy()) {
			float use = this.powerProvider
					.useEnergy(
							1.0F,
							(float) (getMaxWorkEnergy() - getWorkEnergy() / 2.5D),
							true);
	...
	public void setUEwireConnection() {
		if (MFFSConfiguration.MODULE_UE) {
			ElectricityConnections.registerConnector(this, EnumSet
					.of(ForgeDirection.getOrientation(getFacing())
							.getOpposite()));
		...
		if (inputTile != null) {
			if ((inputTile instanceof IConductor)) {
				if (getWorkEnergy() >= getMaxWorkEnergy()) {
					...
					((IConductor) inputTile).getNetwork().startRequesting(this,
							10.0D, 120.0D);

					setWorkEnergy((int) (getWorkEnergy() + ((IConductor) inputTile)
							.getNetwork().consumeElectricity(this).getWatts() / 50.0D));
				}
			}
		}
	}

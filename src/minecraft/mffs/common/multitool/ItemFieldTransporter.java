		...
		if (ffworldmap != null) {
			...
			if ((projector != null) && (generator != null)) {
				if (projector.isActive()) {
					...
					if (passTrue) {
						...
						if ((counter >= 0) && (counter <= 5)) {
							if (((world.getBlockMaterial(x, y, z).isLiquid()) || (world
									.isAirBlock(x, y, z)))
									&& ((world
											.getBlockMaterial(x, y - ymodi, z)
											.isLiquid()) || (world.isAirBlock(
											x, y - ymodi, z)))) {
								if (y - ymodi <= 0) {
									Functions
										...
										stack,
										MFFSConfiguration.forcefieldtransportcost,
										true)) {
									consumePower(
											stack,
											MFFSConfiguration.forcefieldtransportcost,
											false);
									entityplayer.setPositionAndUpdate(x + 0.5D,
											y - ymodi, z + 0.5D);
					...
					&& (!(projector
							.getStackInSlot(projector.getPowerLinkSlot())
							.getItem() instanceof ItemCardPowerLink))) {
				Functions
						.ChattoPlayer(entityplayer,

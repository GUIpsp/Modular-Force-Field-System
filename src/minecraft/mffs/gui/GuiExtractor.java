...
public class GuiExtractor extends GuiMFFS
{
	private TileExtractor tileEntity;
	private boolean editMode = false;

	public GuiExtractor(EntityPlayer player, TileExtractor tileentity)
	{
		super(new ContainerExtractor(player, tileentity), tileentity);
	...
	protected void drawGuiContainerForegroundLayer(int x, int y)
	{
		...
		this.drawTextWithTooltip("progress", "%1: " + (int) (100 - ((float) this.tileEntity.processTime / (float) TileExtractor.REQUIRED_TIME) * 100) + "%", 8, 70, x, y);
	...
	protected void drawGuiContainerBackgroundLayer(float f, int x, int y)
	{
		 ...
		 */
		this.drawSlot(8, 82);
		this.drawBar(30, 84, (float) this.tileEntity.processTime / (float) TileExtractor.REQUIRED_TIME);
